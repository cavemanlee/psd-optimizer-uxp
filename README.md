...
- 文件体积减小时明确显示成功提示，增加时说明可能原因
- `_fix` 副本遵循 Photoshop 的最大兼容性偏好，不再无条件写入兼容合成图
- English / 中文双语界面
- 在说明页手动检查 GitHub Releases 更新，不自动下载或安装文件
...
- 更新检查只在用户点击版本号后发起，关闭说明页或面板会取消请求。
- 只有 Photoshop 的最大兼容性偏好设为“始终”时，自动副本才会写入兼容合成图；“询问”和“从不”不会被插件强制开启。
